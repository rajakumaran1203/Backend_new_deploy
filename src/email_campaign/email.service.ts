...
export class EmailService {
  ...
  //   const description = 'Your scheduled email body text';
...
async scheduleRampUpAndSendEmails(cronJobId: string): Promise<void> {
  ...
  async sendEmail(subject: string, description: string, senderIds:string[]): Promise<void> {
    ...
    const transporter = this.transporters[transporterIndex];
      ...
      const mailOptions = {
        from: senderId,
        to: record.email,
        subject: name.replace(/\${name}/g, name),
        text: description.replace(/\${name}/g, name),
      };
  ...
  // async scheduleFollowup(senderId: string, recipientEmail: string, subject: string, description: string, scheduledAt: Date): Promise<void> {
  //   // Calculate the number of days between the current date and the scheduled date
  //   const daysUntilScheduled = differenceInDays(scheduledAt, new Date());
  
  //   // Convert days to milliseconds
  //   const delayMilliseconds = daysUntilScheduled * 24 * 60 * 60 * 1000;
  
  //   setTimeout(async () => {
  //     await this.sendSingleEmail(senderId, recipientEmail, subject, description);
  //     await this.saveEmailInDatabase(senderId, recipientEmail, subject, description);
  //   }, delayMilliseconds);
  // }
